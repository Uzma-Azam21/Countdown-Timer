...
async function getTimerInput(): Promise<TimerInput> {
...
async function startCountdown(duration: TimerInput): Promise<void> {
  let totalSeconds =
  ...
  const countdown = () => {
    const interval = setInterval(() => {
      ...
      console.log(
        chalk.bold(
          `Time remaining: ${hours}h ${formatTimeUnit(
            minutes
          )}m ${formatTimeUnit(seconds)}s`
        )
      );
    }, 1000);
  };
  ...
  const initialInterval = setInterval(async () => {
    ...
    console.log(
      chalk.bold(
        `\nTime remaining: ${hours}h ${formatTimeUnit(minutes)}m ${formatTimeUnit(
          seconds
        )}s`
      )
    );
